
<?php require("connecte/base.php"); 

$centrex=6;



// si je ne fais pas de retour 
if(!isset($ret1) && $lolo!=1  )
{//$test1.=$mavar4;

}



$Tnumero  = array() ;
    $Treference=array();

 


// declaration et initialisation de variables
if($p==NULL) {
                $p=1; $tab=array();
			 }
			 
// quand on click sur le bouton retour 			 

if(isset($ret1) )
{
if($x>=1)
{
//a chaque fois que je fais un retour j'efface 3 caracteres


$test1=substr($test1, 0, -3);
echo 'caractere supprimés ! '.$test1;

// decrementation de p 
$p=$x;
$numero--;
echo '<br> je suis le code retour <br>';
/* code retour */
if($p==1){
$query11 = " SELECT  * ";
$query11 .= "FROM  argument  ";
$query11 .= "WHERE    numcli='$centrex' and niveau='$p'  ";
}
else {
$query11 = " SELECT  * ";
$query11 .= "FROM  argument  ";
$query11 .= "WHERE    numcli='$centrex' and  reference='$test1'  "; //niveau='$p' 
echo 'la variable de retour que je cherche:'.$test1;
}
$mysql_result11 = mysql_query($query11) or die ("Erreur : ".mysql_error());
while($row11 = mysql_fetch_array($mysql_result11))
{

    $Vheritage = ($row11["commentaire"]);
	
}
$monretour=1;


echo '<br> la variable retour='.$monretour; 
}
else echo "<br> vous ne pouvez pas aller au dela du niveau 0 !";
/* fin code retour */
}
// si je ne suis pas en retour 
else {

require("memorisation.php");




}

//affichage x et p
echo '<br> variable x='.$x.'p='.$p;


// boucle principale
for ($j=0; $j<1; $j++){


/*
//commandes sql
$query1  = " SELECT  commentaire ";
$query1 .= "FROM  argument  ";
$query1 .= "WHERE reference='$toto' ";

$mysql_result = mysql_query($query1) or die ("Erreur : ".mysql_error());
while($row = mysql_fetch_array($mysql_result))
{
    $Targument1 = ($row["commentaire"]);
}
*/
// concatenation de la reference !

?>
<table>
<!-- form principale -->
<form method="POST" action="jaafar.php"> 

<!-- affichage de la requete mysql -->
<tr>
<td>
<input type="text" name=<?php echo 'entry'.$p; ?> value="<?php echo $Vheritage;?>" >
<!-- declaration de p de type hidden pour la sauvegarde et l'envoi -->
<input type="hidden" id="p" name="p" value=<?php  echo $p+1;?> >
<!-- variable retour -->
<input type="hidden"  name="lolo" value="<? echo $monretour; ?>" >
</td>
<!-- variable colonne -->



<td>

<!-- deux boutons oui et non leurs noms est clicki --> 
<input type="submit" name=<?php echo 'clik'.$p; ?> value="oui">
<input type="submit" name=<?php echo 'clik'.$p; ?> value="non">

<!-- compte le nombre de reponses -->
<input type="hidden" name="oui" value=<?php  $l=$p-1;$vari="clik";$reponse=${$vari.$l};if($reponse=="oui") {echo $oui+1;} else echo $oui;?> >
<input type="hidden" name="non" value=<?php $l=$p-1; $vari="clik"; $reponse=${$vari.$l}; if($reponse=="non") {echo $non+1;}else echo $non; ?> >
<input type="submit" name=<?php echo 'clik'.$p; ?> value="ind" >
<!-- envoie de la variable mavar4 -->
 <input type="hidden" name="mavar4" value="<?php echo $_POST['mavar4']; ?>" >
<input type="hidden"  name="test" value="<?   echo $mavar4; ?>" >
<?// echo "<br><br> La variable que je viens de creer =".$test1.'<br><br>' ; ?>
<input type="hidden"  name="test1" value="<?  if(!isset($ret1) /*&& $lolo!=1 */){$test1.=$mavar4; } echo $test1; ?>" >

</td>
 </tr>

</form>
</table>
<?php 
echo "<br><br> La variable que je viens de creer =".$test1.'<br><br>' ;


/*
//+++++++++++++++++++++++++++++++++++++++++++ refe ici 

// 'l' indice de reponse
$l=$p-1;

$vari="clik";
// 'oui' ou 'non'
$reponse=${$vari.$l};
// condition de reponse
// mavar4 contient la reference !
if($reponse=="oui") {
	$mavar4 ="question".$l;
       $mavar4.=".oui";
	   }
else if($reponse=="non"){
$mavar4="question".$l;
$mavar4.=".non";
}
else if($reponse=="indecie"){
$mavar4="question".$l;
$mavar4.=".jesaispas";
}
//else echo 'erreur reference non modifiée !';
//+++++++++++++++++++++++++++++++++++++++++++ refe ici  */
 
?>
<?php if($p!=1) {//!(isset($ret1) && $x==0)){ ?>
<!-- deuxieme form pour le retour -->
<form method="POST" action="jaafar.php"> 

<input type="hidden"  id="x" name="x" value=<?php echo $p-1;?> >

<input type="hidden"   name="ret1"   value="retour1"     /> 

<input type="hidden" name="mavar4" value=<? print("$mavar4"); ?> >
<input type="hidden" name="toto" value=<? print("$mavar4"); ?> >
 <input type="hidden"  name="test1" value="<?   echo $test1; ?>" >
 <input type="submit"   id="ret"   value="Retour"     /> 

</form>
<?php } ?>
<?php



echo "<br> mavar4=".$mavar4."<br>";

// mavar3 contient le commentaire
$e=$p-1;
$mavar="entry";
$mavar2="question";
$mavar3=${$mavar.$e};
echo '<br>  p='.$p;



////////////////////





$query1 = " SELECT  commentaire ";
$query1 .= "FROM  argument  ";
$query1 .= "WHERE reference='$mavar4' ";

$mysql_result = mysql_query($query1) or die ("Erreur : ".mysql_error());
while($row = mysql_fetch_array($mysql_result))
{
    $Targument = ($row["commentaire"]);
}
///////


///////



// je mets le commentaire dans la base si il n'est pas null ou vide !
// je met Vheritage dans la base si ce dernier est null !
if( ( $mavar3!="" || $mavar3!= NULL ) && ($Vheritage==""  || $Vheritage==NULL ) )
{

echo '<br> La reference est :'.$mavar4.'<br>';

$l=$p-1;
$test1=substr($test1, 0, -3);
$query ="INSERT INTO argument (numcli,commentaire,reference,niveau) ";
$query .=" VALUES ('$centrex','$mavar3','$test1','$l')";
$result = mysql_query($query) or die ("Erreur : ".mysql_error()) ;
echo '<br> ajout avec succès <br>';


// condition sans retour
echo "<br><br> LE RETOUR AVANT LABOUCLE =".$lolo.'<br><br>' ;
if($lolo!=1)
{
//ajout de l heritage
require("sans_ret.php");


}
else{
require("ret.php");

echo '<br> Je N UTILISE PAS LE REQUIRE !! <br>';
}


}
else '<br> erreur ajout impossible';

//************************La modification***********************//
// pour le modification $Vheritage==commentaire est non null !
if( ( $mavar3!="" || $mavar3!= NULL ) && ($Vheritage!=""  || $Vheritage!=NULL ) )
{
require("modification.php");

}

mysql_close();

}
?>
